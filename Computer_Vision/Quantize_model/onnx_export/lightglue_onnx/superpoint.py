...
def top_k_keypoints(
    ...
    # Unconditional -- no data-dependent branch -- so this traces to a single
    # fixed-shape TopK instead of an `If` node whose two branches (real N vs
    # fixed k) have irreconcilable output shapes under a TensorRT
    # optimization profile (the pytorch-original "if k >= N: return as-is
    # else topk" pattern builds fine in ONNX Runtime but fails TensorRT
    # engine building with "dimensions not compatible for if-conditional
    # outputs"). Always pad up to k dummy candidates (score = -1, below any
    # real post-softmax/NMS score, which lies in (0, 1]) before taking the
    # top-k, so the result is always exactly k long: real keypoints when
    # there are >= k of them, real + padding otherwise. Callers MUST drop
    # entries with score <= 0 -- see SP_LG.py's _extract_region_onnx.
    pad_kpts = keypoints.new_zeros((k, 2))
    pad_scores = scores.new_full((k,), -1.0)
    keypoints = torch.cat([keypoints, pad_kpts], dim=0)
