...
CONFIG = {
    ...
    'feature_method': 'sift',       # 'sift' | 'akaze' | 'orb' | 'brisk'
    'lowe_ratio': 0.75,
...
def select_descriptor(image, method='sift'):
    ...
    if method == 'sift':
        ...
        detector = cv2.ORB_create(nfeatures=5000, nlevels=8)
    elif method == 'brisk':
        detector = cv2.BRISK_create(thresh=15)
...
def _build_matcher(method):
    """Create the right BFMatcher for the given descriptor type."""
    if method in ('sift', 'akaze'):     # FIX: was `method == 'sift' or 'akaze'`
...
def keypoints_matching(feat1, feat2, method, ratio=0.75):
    bf  = _build_matcher(method)
