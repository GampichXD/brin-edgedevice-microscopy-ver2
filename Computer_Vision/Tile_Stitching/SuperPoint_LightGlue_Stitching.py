...
def build_extractor_matcher(cfg=None):
    ...
    SuperPoint produces:
        keypoints       : (N, 2)   pixel coords
        descriptors     : (N, 256) float32  -- 256-dim learned descriptors
        keypoint_scores : (N,)     detector confidence scores

    Both models are placed on the same device and remain in eval mode
    ...
    matcher = (
        LightGlue(features='superpoint',           # must match extractor type
    ...
    print("[INFO] SuperPoint extractor and LightGlue matcher ready.")
